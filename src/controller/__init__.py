...
class RecommenderEngine:
    ...
    def search_offer(self, query: str, user: Developer):
        ...
        return OfferModelStub().similar_items(user)
...
class UpdateEngine:
    ...
    def update(self, item: Item):
        if isinstance(item, Offer):
            OfferModelStub().update_item(item)
        elif isinstance(item, Developer):
    ...
    def remove(self, item: Item):
        if isinstance(item, Offer):
            OfferModelStub().remove_item(item)
        elif isinstance(item, Developer):
